...
```jldoctest
# A demo on `map(sqrt, (2,))`
Base.print_statement_costs(stdout, map, (typeof(sqrt), Tuple{Int},))

# output

julia> Base.print_statement_costs(stdout, map, (typeof(sqrt), Tuple{Int},))
map(f, t::Tuple{Any}) in Base at tuple.jl:169
1 ─  0 %1  = Base.getfield(_3, 1, true)::Int64
│    1 %2  = Base.sitofp(Float64, %1)::Float64
│    2 %3  = Base.lt_float(%2, 0.0)::Bool
└──  0       goto #3 if not %3
2 ─  0       Base.Math.throw_complex_domainerror(:sqrt, %2)::Union{}
└──  0       unreachable
3 ─ 20 %7  = Base.Math.sqrt_llvm(%2)::Float64
└──  0       goto #4
4 ─  0       goto #5
5 ─  0 %10 = Core.tuple(%7)::Tuple{Float64}
└──  0       return %10
```

This includes the consequences of inlining and other forms of optimization.
